...
public class AdminContextInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		if (!user.getAdmin()) {
			request.setAttribute(MESSAGE,
					MessageResolver.getMessage(request, "login.notAdmin"));
		...
		if (!user.getSites().contains(site)) {
			request.setAttribute(MESSAGE,
					MessageResolver.getMessage(request, "login.notInSite"));
			response.sendError(HttpServletResponse.SC_FORBIDDEN);
			return false;
		}
	...
	private String getLoginUrl(HttpServletRequest request) {
		...
		if (!StringUtils.isBlank(processUrl)) {
			buff.append("&").append(PROCESS_URL).append("=")
	...
	private static String getURI(HttpServletRequest request)
		...
		if (uri.equalsIgnoreCase("/houtai"))
			return null;
		...
		if (start <= 0) {
			throw new IllegalStateException(
					"admin access path not like '/xxxxx/...' pattern: "
							+ uri);
		}
